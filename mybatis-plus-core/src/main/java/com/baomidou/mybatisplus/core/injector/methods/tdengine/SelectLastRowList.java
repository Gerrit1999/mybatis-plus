 ...
 * @since 2023/8/15
 */
public class SelectLastRowList extends AbstractMethod {

    public SelectLastRowList() {
        super(TDengineSqlMethod.SELECT_LAST_ROW_LIST.getMethod());
    }

    @SuppressWarnings("Duplicates")
    @Override
    public MappedStatement injectMappedStatement(Class<?> mapperClass, Class<?> modelClass, TableInfo tableInfo) {
        TDengineSqlMethod sqlMethod = TDengineSqlMethod.SELECT_LAST_ROW_LIST;
    ...
    protected String sqlSelectColumns(TableInfo table, boolean queryWrapper) {
        ...
        return Arrays.stream(selectColumns.split(COMMA))
                .map(col -> String.format("LAST_ROW(%s)", col))
