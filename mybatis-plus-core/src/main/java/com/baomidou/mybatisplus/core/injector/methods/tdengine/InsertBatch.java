...
public class InsertBatch extends AbstractMethod {
    ...
    public InsertBatch() {
        super(TDengineSqlMethod.INSERT_BATCH.getMethod());
    }
    ...
    public MappedStatement injectMappedStatement(Class<?> mapperClass, Class<?> modelClass, TableInfo tableInfo) {
        ...
        // 标签列值
        String tagSqlProperty = this.filterTableFieldInfo(tagFields, null, i -> i.getInsertSqlProperty(LIST + "[0]" + DOT), EMPTY);
        String tagScript = LEFT_BRACKET + tagSqlProperty.substring(0, tagSqlProperty.length() - 1) + RIGHT_BRACKET;
        // 普通列值
        String columnSqlProperty = tableInfo.getKeyInsertSqlProperty(true, ENTITY_DOT, false) +
                this.filterTableFieldInfo(columnFields, null, i -> i.getInsertSqlProperty(ENTITY_DOT), EMPTY);
        String columnScript = SqlScriptUtils.convertTrim(columnSqlProperty, LEFT_BRACKET, RIGHT_BRACKET, null, COMMA);
        columnScript = SqlScriptUtils.convertForeach(columnScript, LIST, null, ENTITY, SPACE);

        // 构造子表名
        Map<String, String> valuesMap = new HashMap<>();
        for (TableFieldInfo tagField : tagFields) {
            String property = tagField.getInsertSqlProperty(LIST + "[0]" + DOT);
